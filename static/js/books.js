...
const editBook = async id => {
  const book = await window.fetch(`/books/get/${id}`)
    .then(res => res.json())

  book.fail
    ? window.alert(book.err)
    : book.consumer_username
      ? window.alert('No puedes editar un libro ya alquilado.')
      : main.innerHTML += `
      <section id="editSection" class="fixed flex bg-black bg-opacity-80 top-0 left-0 w-full h-screen items-center justify-center">
        ...
        class="border-4 border-black bg-orange-200 p-8 rounded-xl font-semibold min-h-max grid grid-cols-4 gap-2">
          ...
          <h4 class="text-2xl underline col-span-3">Editar libro:</h4>
...
const removeBook = async (id, owner_username) => {
  await window.fetch(`/books/${owner_username}/${id}`, { method: 'DELETE' })
